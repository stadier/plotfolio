...
### Estate / Developer Tooling

- [x] **47. Estate / Multi-Unit Hierarchy** — Parent–child Property model: `parentPropertyId`, `unitLabel`, `isContainer`, optional `containerKind` ("estate" | "phase" | "building" | "subdivision"). Reuses Sale/Offer/InstallmentPlan/OwnershipRecord per unit. Tree depth flexible (estate→unit, estate→phase→unit, building→floor→apartment). Same data model serves Nigerian estates, US subdivisions, Dubai towers — labels live only in UI.
- [x] **48. Bulk Unit Creation UX** — Template + N copies + auto-numbering ("Block C / Plot 14") + grid placement on master plot map. Highest-leverage screen for developer onboarding; should let a developer create 60 units in under a minute. _(Initial version landed: BulkUnitCreator modal + `/api/properties/bulk` endpoint; grid-on-map placement still pending.)_
- [ ] **49. Estate Container Extension** — When `isContainer=true`, expose `developerName`, `developerProfileId`, `amenities[]`, `masterPlanDocumentId`, `paymentPlanTemplates[]`, `defaultReservationDeposit`, `progressOverall`. Hidden on leaf units. Estate detail page with tabs (Units, Sales, Documents, Finance).
- [ ] **50. Off-Plan Progress Tracking** — `structure.completionPercent` (0–100) + `structure.expectedHandoverDate`. Marketplace shows "Off-plan" / "Ready" badges and handover ETA. Required disclosures when listing pre-completion units.
- [ ] **51. Reservation Booking Flow** — Add `BookingType.RESERVATION` to existing Booking model + new `PropertyStatus.RESERVED`. Soft-hold with deposit, expires if no offer follows; deposit refundable per developer's `defaultReservationDeposit` config.
- [ ] **52. Buyer Payment-Plan Template Picker** — Offer modal lets buyer pick from developer's `paymentPlanTemplates[]` (e.g. "outright -10%", "12mo 0%", "24mo +8%") with inline schedule preview. Developer can still accept/counter individual offers.
- [ ] **53. Cross-Funding Dashboard (monetization hook)** — Derived view on estate container: aggregate received installments + outright payments across child units, compare to per-unit `completionPercent` and developer-set `constructionBudget`. "₦ collected → % of construction funded" per phase / per estate. **Locked behind on-platform payments** to drive transaction take-rate (TODO #46(j)).
- [ ] **54. Milestone-Release Escrow (monetization hook)** — For off-plan installments: hold buyer payments until structural milestones (foundation, roofing, finishing) verified via developer upload + optional inspector sign-off. Reuses dispute evidence pipeline. Buyer trust unlock that justifies platform cut to both sides.
- [ ] **55. Auto-Generated Payment-Event Documents (monetization hook)** — On each platform installment / outright payment: emit receipt PDF, allocation letter, draft deed-of-assignment, completion certificate. Watermarked + sealed via existing `contractTemplates.ts`. Document type extensions per country via `countryLegalConfig.ts`. Off-platform = developer keeps doing this manually.
- [ ] **56. Public Estate Progress Page (monetization hook)** — Marketing page per estate showing "X of Y units sold, Z% collected, on-track for handover {date}", aggregated milestone progress, anonymized cohort signals. Transparency as sales weapon; only renders credibly when ledger lives on platform.
- [ ] **57. Buyer Credit Profile** — On-time installment history → buyer reputation score on `User`. Unlocks better payment plans + mortgage partner intros (TODO #58). Drives buyer-side pull for on-platform payments (buyer wants the credit history).
- [ ] **58. Mortgage Lead Tracking** — `MortgageLead` sub-doc on `Sale`: `partnerName`, `partnerContact`, `status` (introduced | pre_approved | approved | declined), `referralFeePercent`. Lightweight tracking; loan stays off-platform. Feeds TODO #46(h) revenue line.
- [ ] **59. Marketplace Estate Ranking** — Estate cards outrank child unit cards in default browse to prevent flooding. Child units surface when filters narrow (price band, bedrooms, ready-to-move). Estate card shows aggregate signal: "60 units from ₦45M, 40% sold".
